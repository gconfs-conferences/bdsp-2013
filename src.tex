...
\begin{enumerate}
    ...
    \url{http://github.com} (mais essayez que tous les membres de votre équipe
    s'inscrivent au même endroit, plus simple). \\
    ...
    de base; il faut un compte étudiant. Préférez BitBucket si vous voulez
...
\begin{enumerate}
    \item TextureManager : charger les textures à dessiner
...
\textbf{Répartissez vous les tâches, et c'est parti !}
...
Pour les télécharger, rendez vous ici : \url{http://too.gy/bdsp/tp.zip}. \\
...
Notre projet va contenir 4 classes : \\

\begin{itemize}
    \item[\textbf{Game1}] est la classe créée par XNA, elle est déja présente
        dans le projet. Elle contient les fonctions d'initialisation, de
        chargement du contenu et les fonctions principales de notre jeu, qui
        tournent en boucle ( Draw() et Update() ).
    \item[\textbf{TextureManager}] est la classe qui va s'occuper de charger
        et de contenir nos textures.
    \item[\textbf{Plate}] est la classe qui représentera notre plateau de jeu
        (la map avec des cases sur laquelle nos pièces vont se placer).
    \item[\textbf{Piece}] est la classe qui représentera notre pièce. Cette
        pièce prendra une forme aléatoire parmi les 6 possibles.
\end{itemize}

\subsection{TextureManager : chargement des textures}
...
\begin{csharpcode*}{label=TextureManager.cs}
...
public static class TextureManager
{
    ...
    /* On déclare une fonction qui va charger nos textures et qu'on appellera
       au lancement du jeu */
...
Il faut appeler la fonction LoadContent de notre TextureManager depuis la
...
Le plus simple c'est de représenter les cases notre plateau par une matrice (ou
tableau à deux dimensions) de bytes : \\
...
\begin{csharpcode}
Plate p = new Plate(); // instanciation d'un plateau de jeu
...
Au lancement du jeu, on va instancier nos objets, créer tous ce dont nous
allons avoir besoin par la suite. On va passer l'état du jeu à 'Running', car
c'est le cas et on va redimentionner la fenêtre pour que ça corresponde à la
taille de notre image GameBackground. \\
...
public class Game1
{
    ...
    {
        // TODO: renvoie vrai si la partie est terminée, faux sinon.
    }
